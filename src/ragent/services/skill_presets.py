...
_SKILL_MANAGER_INSTRUCTIONS = (
    ...
    "`readonly` flag.\n"
    "- get_skill(skill_id | skill_name) — view one skill's full details.\n"
    "- update_skill(skill_id | skill_name, name, description, instructions, "
    "enabled) — full replace of an existing skill: you must send ALL fields, so "
    "read the current values with get_skill first and carry over anything you're "
    "not changing (set `enabled` to show/hide it).\n"
    ...
    "- Users refer to skills by NAME — NEVER ask the user for a skill_id. Pass "
    "the name they used as `skill_name` (case-insensitive), or call list_skills "
    "yourself to browse; never guess an id. Only ask the user to clarify when "
    "the name matches nothing in list_skills.\n"
