...
| `DOCUMENT_FORBIDDEN`                 | 403             | `/retrieve/v2` or `/mcp/v2` request targets a `document_id` that does not exist or belongs to another user (anti-IDOR; missing ids are treated the same as foreign ones to avoid existence-oracle leaks) | `RetrieveV2Service.assert_owner` |
