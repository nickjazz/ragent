...
| T-CA.R8 | Behavioral | • **Achieve:** `POST /chatagent/v1` accepts `stream: bool` and `node_filter: str \| None`; when `stream=True`, upstream SSE response is proxied with per-event filtering on `returnData.messages[*].messageMeta.langgraph_node`; `node_filter=null` → all events forwarded; non-streaming path unchanged.<br>• **Deliver:** two fields in `schemas/chatagent.py`; streaming block + filter in `routers/chatagent.py`; 2 schema unit tests; 6 integration tests. | [x] | Dev |
