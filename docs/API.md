...
`POST /mcp/v1` — Model Context Protocol server (JSON-RPC 2.0, spec `2025-06-18`; `initialize` echoes a supported older revision — `2025-03-26` / `2024-11-05` — when the client requests one). Exposes a `retrieve` tool over the corpus and the `AGENTIC_UI_TOOL` client-side dispatcher (§3.8.3a). Full spec: [`docs/spec/mcp_server.md`](docs/spec/mcp_server.md).

| Method | Purpose |
|---|---|
| `initialize` | Capability negotiation. |
| `notifications/initialized` | Client signals init complete; server returns 204. |
| `tools/list` | Returns the `retrieve` tool (Pydantic-derived `inputSchema`, hand-authored `outputSchema`, `annotations: {readOnlyHint: true}`) and `AGENTIC_UI_TOOL` (`inputSchema` `{tool_name, arguments}`; client-side dispatcher for `/chatagent/v3`, §3.8.3a). |
| `tools/call` | Invokes `retrieve`. Result `structuredContent.sources` is the machine-readable source list (for the frontend's retrieved-sources panel); `content[0].text` is a `<context>`-wrapped markdown citation table + `### [N]` excerpt blocks for LLM grounding (no internal fields like `document_id`/`score`; cells injection-safe — CR/LF stripped, `\|` escaped; only http(s) `source_url` linkified with markdown-breaking chars percent-encoded; literal `<context>` tags in corpus text neutralised). Unknown args → `-32602 MCP_TOOL_INPUT_INVALID`. A `tools/call` for `AGENTIC_UI_TOOL` returns a soft `{isError: true}` result — it is client-side and executed by the frontend, never server-side. |
| `ping` | Returns `{}`. |
