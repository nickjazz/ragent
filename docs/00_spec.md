...
|---|---|---|---|---|
| **Authentication** | Who is the caller? | JWT verified by **Armasec** against OIDC `ARMASEC_DOMAIN` (JWKS signature + `iss` + `aud` + `exp`) → `user_id = <RAGENT_JWT_CLAIM_USER_ID>` claim | OFF — `<RAGENT_USER_ID_HEADER>` trusted, validated non-empty | Armasec FastAPI middleware verifies on every protected endpoint; `RAGENT_TRUST_X_USER_ID_HEADER=true` falls back to header (dev/integration override) |
...
**P2 additions:**
- **JWT:** standard OIDC token. Carried in the `<RAGENT_JWT_HEADER>` request header as a **raw token, no `Bearer ` prefix** — the P1 header convention is preserved so clients sending a raw JWT remain compatible. The middleware prepends `Bearer ` internally before calling Armasec's `TokenManager.extract_token_payload`. **Armasec** (`armasec` package) verifies: signature against the JWKS published at `{scheme}://<ARMASEC_DOMAIN>/.well-known/jwks.json` (scheme = `https` unless `ARMASEC_USE_HTTPS=false`), `iss == {scheme}://<ARMASEC_DOMAIN>/`, `aud == <ARMASEC_AUDIENCE>`, `exp` ≥ now. OIDC discovery + JWKS are fetched at composition (`build_container()`) so a misconfigured `ARMASEC_DOMAIN` aborts startup rather than 500-ing the first protected request; the fetched JWKS is then cached in-process for the verifier's lifetime. Cache reuse (no JWKS fetch on every protected request) is a contract requirement pinned by T8.1a tests. Once verified, `<RAGENT_JWT_CLAIM_USER_ID>` (default `preferred_username`) is extracted as the downstream `user_id`. Failure mapping: absent header / malformed token / bad signature / wrong `iss` / wrong `aud` / non-numeric `exp` / `nbf` in future / unknown `kid` / unsupported `alg` / any other Armasec verification error → 401 `AUTH_TOKEN_INVALID`; expired → 401 `AUTH_TOKEN_EXPIRED`; missing required claim → 401 `AUTH_CLAIM_MISSING`. `RAGENT_TRUST_X_USER_ID_HEADER=true` (non-prod only) bypasses JWT verification and reads `<RAGENT_USER_ID_HEADER>` directly.
- **Public paths (Armasec short-circuit):** the JWT middleware does not read the header or call Armasec for `/livez`, `/readyz`, `/startupz`, `/metrics`, `/docs`, `/docs/oauth2-redirect`, `/redoc`, `/openapi.json`. The set is a strict superset of `middleware/logging.py::_SKIP_PATHS` (which covers only the four operational health/metrics paths) — extended here with the FastAPI auto-docs surface. These endpoints are declared `P1 Auth = none` in §4.1 and remain auth-free in P2. The outbound MCP HUB (`src/ragent/mcp_hub/server.py`) runs as a separate process and is not covered by this middleware.
...
- **Auth:** `<RAGENT_JWT_HEADER>: <raw-jwt>` (P2.2 onwards, Armasec-verified per §3.5) or `<RAGENT_USER_ID_HEADER>` fallback (`RAGENT_TRUST_X_USER_ID_HEADER=true`, dev only). Auth applies before JSON-RPC dispatch; failure returns HTTP 401 with `application/problem+json` (NOT a JSON-RPC error — auth is a transport-layer concern).
...
| `AUTH_TOKEN_EXPIRED`                 | 401             | JWT `exp` claim is in the past (raised through Armasec's verification path, T8.1a) | Auth middleware T8.2a |
| `AUTH_CLAIM_MISSING`                 | 401             | `<RAGENT_JWT_CLAIM_USER_ID>` claim absent or empty after Armasec verification (T8.1a) | Auth middleware T8.2a |
| `AUTH_TOKEN_INVALID`                 | 401             | JWT header absent, token malformed, signature mismatch, wrong `iss`, wrong `aud`, or any other Armasec verification failure outside expiry/missing-claim (T8.1a) | Auth middleware T8.2a |
...
| `ARMASEC_DOMAIN`                      | (required when `RAGENT_AUTH_DISABLED=false`) | **P2 only.** OIDC issuer domain. Armasec fetches JWKS from `{scheme}://<ARMASEC_DOMAIN>/.well-known/jwks.json` and validates `iss == {scheme}://<ARMASEC_DOMAIN>/`. |
| `ARMASEC_AUDIENCE`                    | (required when `RAGENT_AUTH_DISABLED=false`) | **P2 only.** Expected `aud` claim. Tokens with mismatched `aud` → 401 `AUTH_TOKEN_INVALID`. |
| `ARMASEC_USE_HTTPS`                   | `true`           | **P2 only.** Scheme toggle for the OIDC discovery + JWKS URL. Set `false` ONLY for in-cluster discovery or local fixture; production deployments MUST keep `true`. |
