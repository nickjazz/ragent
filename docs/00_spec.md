...
**P2 additions:**
- **JWT:** standard OIDC token. Carried in the `<RAGENT_JWT_HEADER>` request header as a **raw token, no `Bearer ` prefix** — the P1 header convention is preserved so clients sending a raw JWT remain compatible. The middleware prepends `Bearer ` internally before calling Armasec's `TokenManager.extract_token_payload`. **Armasec** (`armasec` package) verifies: signature against the JWKS published at `{scheme}://<ARMASEC_DOMAIN>/.well-known/jwks.json` (scheme = `https` unless `ARMASEC_USE_HTTPS=false`), `iss == {scheme}://<ARMASEC_DOMAIN>/`, `aud == <ARMASEC_AUDIENCE>`, `exp` ≥ now. JWKS is fetched lazily on first verification and cached in-process for the Armasec instance lifetime. Cache reuse (no JWKS fetch on every protected request) and graceful handling of JWKS key rotation are contract requirements pinned by T8.1a tests; implementation may rely on Armasec's built-in caching or wrap it. Once verified, `<RAGENT_JWT_CLAIM_USER_ID>` (default `preferred_username`) is extracted as the downstream `user_id`. Failure mapping: absent header / malformed token / bad signature / wrong `iss` / wrong `aud` / non-numeric `exp` / `nbf` in future / unknown `kid` / unsupported `alg` / any other Armasec verification error → 401 `AUTH_TOKEN_INVALID`; expired → 401 `AUTH_TOKEN_EXPIRED`; missing required claim → 401 `AUTH_CLAIM_MISSING`. `RAGENT_TRUST_X_USER_ID_HEADER=true` (non-prod only) bypasses JWT verification and reads `<RAGENT_USER_ID_HEADER>` directly.
