...
- **Rule: Every post-squash migration needs its own chained alembic revision — never edit `migrations/schema.sql` alone.**
  - `alembic/versions/000_squash.py` (`revision="squash"`) reads `migrations/schema.sql` at runtime, but its `upgrade()` only ever executes once — the first time a DB applies the `squash` revision. A DB that already recorded `alembic_version='squash'` is, from alembic's point of view, already at head; editing `schema.sql` afterwards changes nothing for that DB. `alembic upgrade head` only replays revisions between the DB's current revision and head — it does not re-diff file contents.
  - **Action**: any change to `migrations/schema.sql` made after a `migrations/0NN_*.sql` file ships MUST be paired, in the same commit, with a new `alembic/versions/0NN_*.py` file whose `down_revision` points at the current head (chained after `squash`, or after the prior `0NN`). The Python wrapper reads its own `migrations/0NN_*.sql` via `iter_statements` at runtime (mirrors `000_squash.py`) rather than inlining SQL, so the wrapper and the changelog file cannot drift apart.
  - **Verification**: `tests/unit/test_alembic_migration_parity.py::test_every_post_squash_migration_has_a_chained_revision` statically checks, at the filesystem level, that every `migrations/0NN_*.sql` file numbered after the squash cutoff has a chained `alembic/versions/0NN_*.py` revision reaching head. This must be a **static** check rather than a DB-replay test: `squash.upgrade()` re-reads `migrations/schema.sql` at the moment it runs, so any fresh-install replay (including `test_schema_sql_equals_alembic_head`) always "self-heals" from whatever `schema.sql` currently says — it can never detect a missing chained revision, because that gap only manifests on an already-squashed DB, not on a fresh one.

- **Rule: Defensive DDL — every post-squash DDL statement must be idempotent.**
  - `migrations/schema.sql` already bakes every table/column/index into its `CREATE TABLE` statements, and `000_squash.py` dynamically re-reads `schema.sql` on every fresh install. A new database walking the full chain `squash → 0NN → 0NN+1 → ...` therefore gets each object from `squash` first, before any chained `0NN` revision runs its own DDL against it.
  - **Action**: every additive DDL statement in a post-squash migration (alembic revision and/or its paired `migrations/0NN_*.sql`) MUST use `IF NOT EXISTS` (`CREATE TABLE IF NOT EXISTS`, `ADD INDEX IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`, etc.), and every `downgrade()`/drop statement MUST use `IF EXISTS` (`DROP TABLE IF EXISTS`, `DROP INDEX IF EXISTS`, etc.). Without this, a fresh install crashes with a duplicate-object error the moment a chained revision tries to (re-)create something `squash` already created from current `schema.sql`.
